...
class YouTubeService():
    ...
    def getVideo(self):
        try:
            ...
            return youTubeVideo.streams.get_highest_resolution().download(output_path = self.outputPath, filename = uuid.uuid4().hex), FileUtils.clearFileName(youTubeVideo.title)
    ...
    def getAudio(self):
        try:
            ...
            return FileUtils.convertToMP3(youTubeAudio.streams.get_audio_only().download(output_path = self.outputPath, filename = uuid.uuid4().hex)), FileUtils.clearFileName(youTubeAudio.title)
